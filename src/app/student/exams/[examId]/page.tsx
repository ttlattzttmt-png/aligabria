...
import { collection, addDoc, doc, getDoc, getDocs, query, orderBy, where, updateDoc } from 'firebase/firestore';
...
export default function TakeExamPage() {
  ...
  const previousAttemptsRef = useMemoFirebase(() => {
    ...
    return query(collection(firestore, 'students', user.uid, 'quiz_attempts'), where('courseContentId', '==', examId));
  ...
  const handleSubmit = async () => {
    if (isSubmitting || !questions || !user || !firestore || !courseId) return;
    ...
    try {
      ...
      for (const q of questions) {
        ...
        if (q.questionType === 'MCQ') {
          ...
          if (correctOpt && correctOpt.id === ans.mcqOptionId) { correct = true; points = (Number(q.points) || 0); scoreAchieved += points; }
        }
        submissionAnswers.push({ questionId: q.id, questionType: q.questionType, mcqSelectedOptionId: ans.mcqOptionId || null, essayAnswerText: ans.essayText || '', isCorrect: correct, scoreAchieved: points, maxPoints: (Number(q.points) || 0) });
      }
      ...
      // تحديث الإنجاز فوراً بعد تسليم الامتحان
      const allContentSnap = await getDocs(collection(firestore, 'courses', courseId, 'content'));
      const visibleItems = allContentSnap.docs.filter(d => d.data().isVisible !== false);
      
      const vProgSnap = await getDocs(collection(firestore, 'students', user.uid, 'video_progress'));
      const doneVideoIds = new Set(vProgSnap.docs.filter(d => d.data().courseId === courseId).map(d => d.data().contentId));
      
      const attSnap = await getDocs(collection(firestore, 'students', user.uid, 'quiz_attempts'));
      const doneExamIds = new Set(attSnap.docs.filter(d => d.data().courseId === courseId).map(d => d.data().courseContentId));
      
      // نضمن احتساب الامتحان الحالي الذي تم تسليمه للتو
      doneExamIds.add(examId as string);

      const totalCompleted = visibleItems.filter(item => doneVideoIds.has(item.id) || doneExamIds.has(item.id)).length;
      const totalItems = visibleItems.length || 1;
      const actualProgress = Math.min(100, Math.round((totalCompleted / totalItems) * 100));

      await updateDoc(doc(firestore, 'students', user.uid, 'enrollments', courseId), {
      ...
      toast({ title: "تم تسليم الامتحان وتحديث التقدم" });
    } catch (e: any) { console.error(e); toast({ variant: "destructive", title: "خطأ في التسليم" }); } finally { setIsSubmitting(false); }
  };
      ...
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4 text-center">
        <Card className="w-full max-w-lg bg-card border-primary/20 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-500">
          ...
          <CardContent className="p-8 md:p-10 space-y-6 md:space-y-8">
            <div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-orange-500/10 flex items-center justify-center mx-auto text-orange-500"><Lock className="w-10 h-10 md:w-12 md:h-12" /></div>
            <div className="space-y-2"><h1 className="text-2xl md:text-3xl font-black">الاختبار مكتمل</h1><p className="text-muted-foreground font-bold text-sm md:text-base">لقد استنفدت محاولتك في هذا الاختبار مسبقاً.</p></div>
            <div className="p-5 md:p-6 bg-secondary/30 rounded-2xl md:rounded-3xl border border-white/5 space-y-2"><p className="text-[10px] font-black text-muted-foreground uppercase">نتيجتك السابقة</p><p className="text-4xl md:text-5xl font-black text-primary">{lastAttempt.score}%</p></div>
      ...
      <Card className="w-full max-w-lg bg-card border-primary/20 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-500">
        ...
        <CardContent className="p-8 md:p-12">
           <div className="text-center mb-8 md:mb-10 space-y-4">
              <div className={cn("w-20 h-20 md:w-24 md:h-24 rounded-full flex items-center justify-center mx-auto", finishedResult.isSuccess ? "bg-accent/10 text-accent" : "bg-destructive/10 text-destructive")}>{finishedResult.isSuccess ? <Trophy className="w-10 h-10 md:w-12 md:h-12" /> : <XCircle className="w-10 h-10 md:w-12 md:h-12" />}</div>
           ...
           {exam?.allowInstantResultsDisplay ? (
              <div className="p-6 md:p-8 bg-secondary/30 rounded-2xl md:rounded-3xl border border-white/5 flex flex-col items-center justify-center mb-8 md:mb-10"><p className="text-5xl md:text-6xl font-black text-primary">{finishedResult.score}%</p><p className="text-[10px] font-black text-muted-foreground mt-2 uppercase">نسبة النجاح</p></div>
           ) : (
             <Card className="bg-primary/5 border-dashed border-primary/20 p-6 md:p-8 text-center rounded-2xl md:rounded-3xl mb-8 md:mb-10"><p className="text-primary font-bold text-sm md:text-base leading-relaxed">تم استلام إجاباتك بنجاح. ستظهر النتائج فور اعتمادها.</p></Card>
           )}
           <Link href="/student/dashboard" className="block"><Button className="w-full h-14 md:h-16 bg-primary text-primary-foreground font-black rounded-xl md:rounded-2xl text-lg shadow-xl">العودة للوحة التحكم</Button></Link>
        </CardContent>
      </Card>
  ...
  if (isCheckingAttempts || isExamLoading || isQuestionsLoading || !currentQ) return (
    <div className="flex flex-col items-center justify-center min-h-screen gap-4"><Loader2 className="w-10 h-10 animate-spin text-primary" /><p className="font-bold text-muted-foreground">جاري التحضير...</p></div>
  );
  ...
  return (
    <div className="min-h-screen bg-background pb-32 text-right">
      <div className="sticky top-0 z-50 bg-card/80 backdrop-blur-xl border-b w-full flex items-center justify-between p-3 md:p-4 px-4 md:px-6 shadow-sm">
          <div className="text-xs md:text-lg font-black bg-primary/10 px-3 md:px-4 py-1.5 md:py-2 rounded-lg md:rounded-xl text-primary flex items-center gap-1.5 md:gap-2 border border-primary/20"><Clock className="w-3.5 h-3.5 md:w-4 md:h-4" /> {timeLeft !== null ? `${Math.floor(timeLeft/60)}:${(timeLeft%60).toString().padStart(2,'0')}` : '--:--'}</div>
          <Button onClick={handleSubmit} disabled={isSubmitting} className="bg-primary hover:bg-primary/90 font-black px-4 md:px-10 h-10 md:h-12 rounded-lg md:rounded-xl text-xs md:text-sm gap-2">{isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <SendHorizontal className="w-4 h-4" />}<span>تسليم</span></Button>
      </div>
      <main className="container mx-auto p-4 max-w-4xl pt-4 md:pt-6 space-y-6">
        <Card className="bg-card border-primary/10 rounded-2xl md:rounded-[2.5rem] p-6 md:p-10 shadow-2xl relative overflow-hidden">
           <div className="flex justify-between items-center flex-row-reverse mb-6"><Badge className="bg-primary/10 text-primary border-primary/20 font-black px-3 md:px-4 py-1 text-[10px] md:text-sm rounded-full">سؤال {activeQuestionIndex + 1} من {questions.length}</Badge><Badge variant="secondary" className="font-bold text-[9px] md:text-xs">{currentQ.points} درجة</Badge></div>
           {currentQ.imageUrl && (<div className="w-full rounded-xl md:rounded-2xl overflow-hidden border-2 border-primary/10 bg-black/5 mb-6 md:mb-8 flex justify-center shadow-inner"><img src={currentQ.imageUrl} alt="" className="w-full h-auto max-h-[400px] md:max-h-[600px] object-contain block" /></div>)}
           ...
           <div className="flex justify-between pt-8 md:pt-10 gap-3 md:gap-4"><Button variant="outline" disabled={activeQuestionIndex === 0} onClick={() => setActiveQuestionIndex(p => p - 1)} className="h-12 md:h-14 flex-1 rounded-xl md:rounded-2xl font-black border-primary/20 text-primary text-xs md:text-base gap-1.5 md:gap-2"><ChevronRight className="w-4 h-4 md:w-5 md:h-5" /> السابق</Button><Button variant="outline" disabled={activeQuestionIndex === questions.length - 1} onClick={() => setActiveQuestionIndex(p => p + 1)} className="h-12 md:h-14 flex-1 rounded-xl md:rounded-2xl font-black border-primary/20 text-primary text-xs md:text-base gap-1.5 md:gap-2">التالي <ChevronLeft className="w-4 h-4 md:w-5 md:h-5" /></Button></div>
        </Card>
